...
class ArduinoService:
    ...
    async def lib_search(self, query: str) -> CommandResult:
        return await self.runner.run(["lib", "search", "--format", "json", query], timeout_seconds=60)

    async def lib_install(self, name: str) -> CommandResult:
        return await self.runner.run(["lib", "install", name], timeout_seconds=300)
