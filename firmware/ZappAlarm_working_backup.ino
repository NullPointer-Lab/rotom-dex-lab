...
bool alarmAlreadyPlayedToday = false;
...
void updateRobotAnimation() {
  ...
  if (eyesOpen && now - lastBlinkTime >= 1200) {
    drawClosedEyes();
    eyesOpen = false;
    lastBlinkTime = now;
  }
  if (!eyesOpen && now - lastBlinkTime >= 180) {
    drawOpenEyes();
    eyesOpen = true;
    lastBlinkTime = now;
  }
...
bool isAlarmDay(struct tm timeinfo) {
  return timeinfo.tm_wday >= 0 && timeinfo.tm_wday <= 6;
}

void checkAlarm(struct tm timeinfo) {
  if (timeinfo.tm_mday != lastAlarmDay) {
    alarmAlreadyPlayedToday = false;
  }

  if (!alarmAlreadyPlayedToday &&
      isAlarmDay(timeinfo) &&
      timeinfo.tm_hour == ALARM_HOUR &&
      timeinfo.tm_min == ALARM_MINUTE) {
    alarmAlreadyPlayedToday = true;
...
void checkStopSensor() {
  ...
  if (sensorActive && !sensorWasActive && now - lastSensorAction >= 600) {
    if (alarmSoundOn) {
      stopAlarm();
    } else if (clockMode) {
      cycleBrightness();
...
void checkConversationSensor() {
  ...
  unsigned long holdTime = conversationMode ? 1600UL : 10000UL;
